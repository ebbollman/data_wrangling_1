...
This function lets us choose some columns and not others The colon
specifies from variable X to variable Y
...
Using select helpers. Helps select columns want to keep based on some
criteria of variable name.
...
Sometimes helpful to know everything will return everything. e.g. want
to look at litter\_number as first variable, and then also return rest
of vars.
...
Relocate function does same thing, re-orders columns with variable
specified to beginning.
...
Because these two functions do not include all observations in df, shows
that some of gd0\_weight have NA in this variable.

More logical operators Two equal signs is test for equality, rather than
one equal sign that changes the variable.
...
How to select multiple values of a variable. Where group is either Con7
or Mod8
...
Can occasionally be helpful to arrange dataset in a certain way if want
to examine it
...
What if want to import dataset, then clean names, then select some
variables, then mutate and then filter… all at once?
...
Can be more specific about where the results of steps in pipe operator
will enter in subsequent code step.
